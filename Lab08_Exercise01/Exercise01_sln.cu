...
void particlesGPU()
{
	...
	thrust::sort_by_key(thrust::device_ptr<int>(d_key_values->sorting_key), thrust::device_ptr<int>(d_key_values->sorting_key + NUM_PARTICLES), thrust::device_ptr<int>(d_key_values->value));
	...
	thrust::exclusive_scan(thrust::device_pointer_cast(d_env->count), thrust::device_pointer_cast(d_env->count + ENV_BINS), thrust::device_pointer_cast(d_env->start_index));
